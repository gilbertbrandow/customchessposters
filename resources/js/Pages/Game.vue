        ...
        <div class="container">
            <ul class="game__collection is--single">
                <li class="single">
                    <div class="content">
                        ...
                        <div class="is--flex is--mobile-reversed">
                            <div class="player">
                                <div class="flag">
                                    <Flags
                                        :country="this.$data.game.white_country"
                                    />
                                    <div class="info">
                                        <div></div>
                                        <span>{{
                                            this.$data.game.white_country
                                        }}</span>
                                    </div>
                                </div>
                                <div class="name">
                                    {{ this.$data.game.white_name }} ({{
                                        this.$data.game.white_rating
                                            ? this.$data.game.white_rating
                                            : "N/A"
                                    }})
                                </div>
                            ...
                            <div class="player">
                                <div
                                    class="flag"
                                    @mouseover="removeClass($event)"
                                >
                                    <Flags
                                        :country="this.$data.game.black_country"
                                    />
                                    <div class="info">
                                        <div></div>
                                        <span>{{
                                            this.$data.game.black_country
                                        }}</span>
                                    </div>
                                </div>
                                <div class="name">
                                    {{ this.$data.game.black_name }} ({{
                                        this.$data.game.black_rating
                                            ? this.$data.game.black_rating
                                            : "N/A"
                                    }})
                                </div>
                        ...
                        <div
                            :class="[
                                'banner',
                                this.$data.game.world_championship_game
                                    ? 'is--icon'
                                    : '',
                            ]"
                        >
                            <Icon
                                v-if="this.$data.game.world_championship_game"
                                name="king"
                            />
                            <span
                                v-text="
                        ...
                        <strong class="is--small">{{
                            this.$data.game.opening_eco +
                            ": " +
                            this.$data.game.opening_name
                        }}</strong>
                    ...
                    <Poster
                        :poster="this.$data.game"
                        :controls="{ save: true, edit: true }"
...
export default {
    ...
    data() {
        return {
            ...
            moveComment: [
                this.$page.props.game.diagram_position,
                this.$page.props.game.move_comment,
            ],
            variant: null,
            total: 0,
            environment: null,
    ...
    methods: {
        makeMove(direction) {
            if (
                direction &&
                this.chessGame.history().length > this.game.diagram_position
            )
                this.game.diagram_position++;
            else if (!direction && this.game.diagram_position > 0)
                this.game.diagram_position--;
        },
        ...
        onKeydown(e) {
            switch (e.key) {
                case "ArrowRight":
                    if (
                        this.game.diagram_position <
                        this.chessGame.history().length
                    )
                        this.game.diagram_position++;
                    break;

                case "ArrowLeft":
                    if (this.game.diagram_position > 0)
                        this.game.diagram_position--;
                    break;
        ...
        recieveVariant(emitted) {
            ...
            if (emitted.frame && emitted.frame)
                this.$data.environment = emitted.frame;
            else this.$data.environment = 0;
        },
    },
    ...
    computed: {
        moveInfo() {
            ...
            let indexOfMove =
                pgn.indexOf(Math.round(this.game.diagram_position / 2) + ".") +
                Math.round(this.game.diagram_position / 2).toString().length +
                2;
            ...
            if (this.game.diagram_position % 2 != 0) {
                ...
                return (
                    ...
                    Math.round(this.game.diagram_position / 2) +
                    ". " +
                    pgn.substring(indexOfMove, spaceIndex)
                );
            } else {
                //Black move
                let nextSpaceIndex =
                    pgn.indexOf(" ", spaceIndex + 1) > 0
                        ? pgn.indexOf(" ", spaceIndex + 1)
                        : pgn.length;
                ...
                return (
                    ...
                    Math.round(this.game.diagram_position / 2) +
                    ". ... " +
                    pgn.substring(spaceIndex + 1, nextSpaceIndex)
                );
            }
        },
    },
    ...
    watch: {
        "game.diagram_position"() {
            ...
            if (this.game.diagram_position != this.moveComment[0])
